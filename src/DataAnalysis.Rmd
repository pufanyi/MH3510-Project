...
$\mathcal{H}_0$: $\mu_{\text{control}} \le \mu_{\text{no control}}$

```{r}
t.test(y_control, y_no_control, alternative = "less")
```
...
```{r}
# bar plot for predicted values, no need to save
barplot(predicted, col = "lightblue", main = "Predicted values")
barplot(y, col = "lightblue", main = "Actual values")
```

```{r, fig.width=5, fig.height=5}
# scatter plot y vs predicted values, no need to save
plot(y, predicted, col = "blue", main = "y vs predicted values")
# y=x
abline(0, 1, col = "red")
```
