...
```{r}
y_prime <- log(data$Y)
fig <- ggplot(data.frame(y_prime), aes(x = y_prime)) +
  geom_histogram(aes(y = after_stat(density)), fill = "lightblue", color = "black", bins = 30, alpha = 0.6) +
  geom_density(color = "red", linewidth = 1) +
  labs(title = "Distribution of log(y)", x = "y'") +
  theme_minimal()

fig

ggsave(file.path(overview_image_path, "log_Y.pdf"), fig, width = 5, height = 3)
```
...
scatter_plot_X1 <- ggplot(data, aes(x = X1, y = Y)) +
...
```{r}
plot_resid_X1 <- function() {
  plot(slr_X1, which = 1)
}

residuals_image_path_X1 <- file.path(X1_output_folder, "residuals.pdf")
draw(plot_resid_X1, residuals_image_path_X1)
```
